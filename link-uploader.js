 ...
 * Drop your PlusVibe lead CSV in, it mints a link for every lead and hands
 * back a CSV with a `signal_link` column to upload to PlusVibe.
 *
 * Runs on your machine only; leads go straight to the Supabase store, nothing
 * is shown in chat.
 */
const http = require('http');
const { buildLinks, DEFAULT_BASE } = require('./build-links');
...
<div class="card">
  ...
  <p>Drop your lead CSV in. Every lead gets its own personalised link in a new <code>signal_link</code> column, ready to upload to PlusVibe.</p>
  ...
  <div class="hint">Minting is free — a lead only costs anything when they actually open their link. Base URL: <code>${DEFAULT_BASE}</code></div>
</div>
<script>
  ...
  go.onclick=async()=>{
    ...
    try{
      const r=await fetch('/process',{method:'POST',headers:{'content-type':'text/csv','x-filename':picked.name},body:text});
      const j=await r.json();
      if(!j.ok) throw new Error(j.error||'failed');
      const blob=new Blob([j.csv],{type:'text/csv'}); const url=URL.createObjectURL(blob);
      out.className='out show';
      out.innerHTML='<div class="stat"><b>'+j.minted+'</b> links minted'+(j.skipped?' &middot; <b>'+j.skipped+'</b> skipped (no email)':'')+'</div>'
...
http.createServer(async (req, res) => {
  ...
  if (req.method === 'POST' && req.url === '/process') {
    ...
    req.on('end', async () => {
      try {
        const { csv, minted, skipped } = await buildLinks(body);
        const inName = (req.headers['x-filename'] || 'leads.csv').replace(/\.csv$/i, '');
        res.writeHead(200, { 'content-type': 'application/json' });
        res.end(JSON.stringify({ ok: true, minted, skipped, csv, outName: `${inName}-with-links.csv` }));
      } catch (e) {
        res.writeHead(200, { 'content-type': 'application/json' });
